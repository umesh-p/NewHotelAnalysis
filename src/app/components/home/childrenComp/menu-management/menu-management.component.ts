...
export class MenuManagement implements OnInit {
  ...
  allMaterialsData: any = [];
	...
	itemIndex = -1;
  viewAddNewMenuPanel = false;
  isEditMaterial = false;
  allUnits = ['Kg', 'Gms', 'Liters'];
  totalPricePerPlate = 0;
  ...
  ngOnInit() {
    ...
    this.menuService.getAll().subscribe((result) => {
      ...
      for (const i in this.allMenuItems) {
  ...
  onSubmit(formData) {
    ...
    const newMenuItem = Object.assign({}, formData.value);
    ...
    if (!newMenuItem.id) {
      ...
      const objIndex = this.allMenuItems.findIndex((obj => obj.id == newMenuItem.id));

      const UpdatedItems = newMenuItem;
  ...
  calculateCost() {
    ...
    for (const i in this.menuItemMaterials) {

      const item = this.menuItemMaterials[i];
      const itemDetails = this.allMaterialsData.filter((material) => material['itemname'] == item['materialname'] )[0];
